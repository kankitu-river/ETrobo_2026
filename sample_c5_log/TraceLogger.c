...
typedef struct {
    ...
    int32_t left_count;//左モータのエンコーダ値[度]
    int32_t right_count;//右モータのエンコーダ値[度]
} trace_log_t;
                        ...
                        int32_t left_count, int32_t right_count)
{
    ...
    log_entry->left_count = left_count;//左モータのエンコーダ値を保存
    log_entry->right_count = right_count;//右モータのエンコーダ値を保存
}
...
void TraceLogger_Dump(void)//ログのダンプ関数
{
    ...
    syslog(LOG_NOTICE, "time_ms,ref,steering,left_count,right_count");//CSV形式のヘッダを出力
    for (i = 0U; i < fg_log_count; i++) {//保存されているログを順番に出力
        log_entry = &fg_logs[i];//ログのポインタを取得
        syslog(LOG_NOTICE, "%u,%d,%d,%d,%d",//CSV形式でログを出力（syslogは引数5個まで）
               (unsigned int)log_entry->elapsed_ms,
               (int)log_entry->reflection,
               (int)log_entry->steering,
               (int)log_entry->left_count,
               (int)log_entry->right_count);
